...
echo "Checking if config file for haproxy exists"
if [ ! -e ${CONFIG} ]; then
  echo "${CONFIG} not found"
  exit 1
fi

echo "Checking if default.pem has been created"
...
echo "Mark Syn Packets"
# for details: https://engineeringblog.yelp.com/2015/04/true-zero-downtime-haproxy-reloads.html
...
echo "Set up the queuing discipline"
...
echo "Create a plug qdisc with 32 meg of buffer"
nl-qdisc-add --dev=lo --parent=1:4 --id=40: plug --limit 33554432
echo "Release the plug"
nl-qdisc-add --dev=lo --parent=1:4 --id=40: --update plug --release-indefinite

echo "Set up the filter, any packet marked with \"1\" will be directed to the plug"
